...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    protected function ensureRoles()
    {
        if (null === $this->_roles) {
    ...
    public static function getAuthRoles()
    {
        ...
        foreach ($roles as $id=>$role) {
            $arr[$id] = \Yii::t('app', $role->description);
        }
        return $arr;
    }
    ...
    public function getAssignment()
    {
        ...
        foreach ($roles as $id=>$role) {
            $arr[$id] = \Yii::t('app/default', $role->description);
        }
        return array_keys($arr);
    }

    public function setAssignment($roles)
    {
    ...
    {
        if ($em = static::findOne(['email' => $this->email])) {
            if ($em->id !== $this->id) {
                $this->addError('email', 'Указанный адрес e-mail уже используется');
            }
        }
    }
